  ...
  smoothedrpm = ((rpm * 0.1 ) + (smoothedrpm * 0.9)); // higly smoothed
    ...
    // current usage for if the board is engadged or not
     ...
     if (shouldGetFloat && UART.getFloatValues()) {
       ...
       if ((current < 2) && (smoothedrpm > -1 && smoothedrpm < 1) &&
           ((dutycycle * 100 > -2) && (dutycycle * 100) < 2) &&
           (imu_inactivity_count > 2)) {
    ...
    if ((!person_on_ui || (free_fall_preset == 250)) || (!person_on_ui && (free_fall_preset == 250))){
      ...
      set_preset();
       ...
       if (should_lights_be_on == false){
          bri = float_pkg && lightIdleBrightness >= 0 ? lightIdleBrightness : 0;
          stateUpdated(1);
         }else{
          bri = float_pkg && lightBrightness >= 0 ? lightBrightness : 255;
          stateUpdated(1);
         }
